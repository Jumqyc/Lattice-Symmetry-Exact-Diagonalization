...
class Lattice:
    ...
    def __init__(self, 
        ...
        A lattice (supercell) is defined by its reciprocal vectors and their multiplicities, as well as additional cell information.
        Arguments:
        - supercell_info: A variable number of tuples, each containing a supercell vector (as a numpy array) and its multiplicity (as an integer).
        - cell_info: Additional information about the cell, provided as keyword arguments. Each key is a string describing the atom, and each value is a numpy array representing the atom's position in the cell.
        ...
        self.unit_cell_idx: list[tuple[np.ndarray, int]] = []
        for n in product(*[range(m) for _, m in supercell_info]):
            for vec in cell_info.values():
        ...
        # extending by PBC:
        self.extended_cell = []
        for pos, idx in self.unit_cell_idx: # to avoid modifying the list while iterating over it
            for n in product(
                        range(-MAX_EXTENSION, MAX_EXTENSION + 1),
                        repeat=self.spacial_dim):
                self.extended_cell.append(
                    (pos + np.array(n) @ self.supercell_vectors, idx)
                )
    def center_at(self, center):
        ...
        if isinstance(center, int):
            ...
            center_pos,_ = self.unit_cell_idx[center]
        else:
            center_pos = np.asarray(center)
        self.extended_cell = [(pos - center_pos, idx) for pos, idx in self.extended_cell]
        self.unit_cell_idx = [(pos - center_pos, idx) for pos, idx in self.unit_cell_idx]
    ...
    def _get_idx(self, pos,tol = 1e-6) -> int:
        for p,idx in self.extended_cell:
            if np.allclose(p, pos, atol=tol):
                return idx
    ...
    def get_permutation_pt(self, arr:np.ndarray)-> np.ndarray:
        ...
        for pos, idx in self.unit_cell_idx:
            ...
            permuted[idx] = self._get_idx(transformed_pos)
    ...
    def get_permutation_tr(self, dir):
        ...
        for pos,idx in self.unit_cell_idx:
            translated[idx] = self._get_idx(pos + self.reciprocal_vectors[dir])
        return translated
